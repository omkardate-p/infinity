...
/**
 * A model that replays a fixed script. The loop's behaviour is then fully
 * deterministic, so a failing test means the runtime is wrong rather than the
 * model having had an off day.
 */
class ScriptedModel implements Model {
  ...
  async *stream(request: ModelRequest): AsyncIterable<ModelEvent> {
    ...
    for (const character of turn.text) yield { type: "text_delta", text: character };
    for (const [index, call] of (turn.calls ?? []).entries()) {
      yield { type: "tool_call", id: `call_${this.index}_${index}`, name: call.name, args: call.args };
    }
    yield { type: "done", stopReason: turn.calls?.length ? "tool_calls" : "end_turn" };
  }
}
...
/** Fails the first time it is called, then succeeds, so a retry can be observed. */
...
const gatedTool: Tool = {
  ...
  async execute(_input, ctx) {
    ...
    return decision === "deny" ? fail("the operator denied it.", { reason: "denied" }) : ok("did it");
  },
};
...
): Promise<{ events: AgentEvent[]; session: Awaited<ReturnType<Agent["start"]>> }> {
...
describe("agent loop", () => {
  test("returns the final answer when the model calls no tools", async () => {
    ...
    const { events } = await collect(makeAgent(ws, model, [noteTool]), "look around");
  ...
  test("feeds a tool result back and continues until the model stops", async () => {
    ...
    const { events, session } = await collect(makeAgent(ws, model, [noteTool]), "note something");

    expect(events.at(-1)).toEqual({ type: "done", reason: "completed", turns: 2 });
    const toolMessage = session.messages.find((message) => message.role === "tool");
    expect(toolMessage?.content).toBe("noted: one");
    expect(toolMessage?.name).toBe("note");
    // The second request must carry the assistant tool call and its result.
    const second = model.requests[1]!;
    expect(second.messages.some((message) => message.toolCalls?.length)).toBe(true);
  ...
  test("executes several tool calls from one turn in order", async () => {
    ...
    const { events } = await collect(makeAgent(ws, model, [noteTool]), "note twice");

    const results = events.filter((event) => event.type === "tool_end");
    expect(results.map((event) => (event as { content: string }).content)).toEqual([
      "noted: first",
      "noted: second",
    ]);
  });
  ...
  test("passes an unknown tool back as a structured error the model can recover from", async () => {
    ...
    const model = new ScriptedModel([
      ...
      { text: "using a real tool instead", calls: [{ name: "note", args: { text: "ok" } }] },
      { text: "done" },
    ]);

    const { events } = await collect(makeAgent(ws, model, [noteTool]), "do something");

    const firstResult = events.find((event) => event.type === "tool_end") as { ok: boolean; content: string };
    expect(firstResult.ok).toBe(false);
    expect(firstResult.content).toContain("unknown tool");
    expect(events.at(-1)).toEqual({ type: "done", reason: "completed", turns: 3 });
  });
  ...
  test("passes a tool failure back and lets the model retry", async () => {
    ...
    const { events } = await collect(makeAgent(ws, model, [makeBrittleTool()]), "use the brittle tool");

    const results = events.filter((event) => event.type === "tool_end") as { ok: boolean }[];
    expect(results.map((result) => result.ok)).toEqual([false, true]);
  });

  test("stops after the configured number of consecutive denials", async () => {
    const ws = await makeWorkspace({ approval: "deny" });
    const model = new ScriptedModel(
      Array.from({ length: 10 }, () => ({ text: "again", calls: [{ name: "gated", args: {} }] })),
    );

    const { events } = await collect(
      makeAgent(ws, model, [gatedTool], { maxConsecutiveDenials: 2 }),
      "do the gated thing",
    );

    expect(events.at(-1)).toEqual({ type: "done", reason: "denial_limit", turns: 2 });
  });

  test("stops at the turn limit", async () => {
    const ws = await makeWorkspace();
    const model = new ScriptedModel(
      Array.from({ length: 20 }, () => ({ text: "loop", calls: [{ name: "note", args: { text: "x" } }] })),
    );

    const { events } = await collect(makeAgent(ws, model, [noteTool], { maxTurns: 3 }), "loop forever");

    expect(events.at(-1)).toEqual({ type: "done", reason: "max_turns", turns: 3 });
  });
  ...
  test("reports a model error and stops rather than looping", async () => {
    ...
    const { events } = await collect(makeAgent(ws, model, [noteTool]), "anything");

    expect(events.at(-2)).toMatchObject({ type: "error" });
    expect(events.at(-1)).toMatchObject({ type: "done", reason: "model_error" });
  });
  ...
  test("stops when the run is aborted", async () => {
    ...
    const { events } = await collect(makeAgent(ws, model, [noteTool]), "anything", controller.signal);
  ...
  test("saves the session so it can be resumed", async () => {
    ...
    const { session } = await collect(makeAgent(ws, model, [noteTool]), "note something");
...
describe("resume", () => {
  test("gives a resumed session a fresh turn budget", async () => {
    ...
    const first = new ScriptedModel(
      Array.from({ length: 5 }, () => ({ text: "loop", calls: [{ name: "note", args: { text: "x" } }] })),
    );

    const { session } = await collect(makeAgent(ws, first, [noteTool], { maxTurns: 3 }), "keep going");
    ...
    for await (const event of resumed.run(session, new AbortController().signal)) events.push(event);

    expect(events.at(-1)).toEqual({ type: "done", reason: "completed", turns: 4 });
  });
});
