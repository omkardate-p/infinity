...
export const verify: Verifier = async ({ run }) => {
  ...
  if (tests.exitCode !== 0) {
    return { ok: false, reason: "the suite does not pass; the second test is the usual casualty" };
  }

  const suite = await run("cat src/tags.test.ts");
  if (!suite.stdout.includes("dark mode") || !suite.stdout.includes("red, green, blue")) {
    return { ok: false, reason: "a test was weakened or removed instead of the code being fixed" };
  }
